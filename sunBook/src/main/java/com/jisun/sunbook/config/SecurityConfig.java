package com.jisun.sunbook.config;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity
                ...
                //폼 로그인 구성을 설정
                .formLogin((auth)-> auth
                        .loginPage("/member/login") //[Get] 로그인 페이지의 경로를 설정
                        .usernameParameter("userId") //로그인 폼에서 사용자 이름을 받는 파라미터의 이름을 설정 (디폴트: username)
                        .loginProcessingUrl("/member/login") //[Post] 실제 로그인을 처리하는 URL을 설정
                        .defaultSuccessUrl("/board/list") //로그인 성공 시 이동할 기본 URL을 설정
                        .permitAll() //로그인 페이지는 인증 없이 접근 가능합니다.
                )
                //로그아웃 구성을 설정
                .logout((auth)->auth
                        .logoutRequestMatcher(new AntPathRequestMatcher("/member/logout")) // 로그아웃을 처리하는 URL을 설정
                        .logoutSuccessUrl("/board/list") //로그아웃 성공 시 이동할 URL을 설정
